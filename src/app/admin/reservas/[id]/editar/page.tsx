...
export default function EditarReservaPage() {
    ...
    if (storedReservations) {
      ...
      if (reservation) {
        setFormData({
          ...
          clientCpf: reservation.clientCpf,
          ...
          paidValue: reservation.paidValue || 0,
          ownerTotalValue: reservation.ownerTotalValue || 0,
          ownerPaidValue: reservation.ownerPaidValue || 0,
        });

        // Atualizar valores formatados
        setDisplayValues({
          totalValue: formatCurrency(reservation.totalValue),
          paidValue: formatCurrency(reservation.paidValue || 0),
          ownerTotalValue: formatCurrency(reservation.ownerTotalValue || 0),
          ownerPaidValue: formatCurrency(reservation.ownerPaidValue || 0),
        });
      }
    }

    // Carregar extras desta reserva
    const storedExtraServices = localStorage.getItem('extraServices');
    if (storedExtraServices) {
      const allExtraServices: ExtraService[] = JSON.parse(storedExtraServices);
      const reservationExtras = allExtraServices.filter(e => e.reservationId === reservationId);
      setExtras(reservationExtras);
    }
  ...
  const calculateOwnerValue = () => {
    ...
    const ownerPercentage = property.ownerPercentage || 70;
    return (formData.totalValue * ownerPercentage) / 100;
  };
  ...
  const handleWhatsAppClick = () => {
    if (!formData.clientPhone) {
      alert('Telefone do cliente não está cadastrado!');
      return;
    }
    ...
    const message = 'Olá, aqui é da Lago Vibes! Estou falando sobre a sua reserva. Aqui estão os detalhes:';
  ...
  const confirmUpdate = () => {
    ...
    // Atualizar extras no localStorage
    const storedExtras = localStorage.getItem('extraServices');
    const allExtras: ExtraService[] = storedExtras ? JSON.parse(storedExtras) : [];
    
    // Remover extras antigos desta reserva
    const filteredExtras = allExtras.filter(e => e.reservationId !== reservationId);
    
    // Adicionar extras atualizados
    ...
    filteredExtras.push(...extrasWithReservation);
    localStorage.setItem('extraServices', JSON.stringify(filteredExtras));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Editar Reserva</h1>
          <p className="text-gray-600">Atualize as informações da reserva</p>
        </div>
      </div>
